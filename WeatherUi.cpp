...
void WeatherUi::drawNoData(TFT_eSPI& spr, int ox, const char* msg) {
...
void WeatherUi::paintFrame(TFT_eSPI& spr, const WeatherModel& w, const PvModel& pv,
  ...
  if (alertActive_) {
    ...
    drawView(spr, prevView_, -off, 1.f, w, pv, hist, fl);
    drawView(spr, view_, W - off, enterT, w, pv, hist, fl);
  } else {
    drawView(spr, view_, 0, enterT, w, pv, hist, fl);
  }
...
bool WeatherUi::render(const WeatherModel& w, const PvModel& pv, const PvHistory& hist,
  ...
  for (int b = 0; b < BAND_N; ++b) {
    ...
    setBand(spr_, top, VIEW_H);
    paintFrame(spr_, w, pv, hist, fl, wifiOk, nowMs);
...
void WeatherUi::drawViewStats(TFT_eSPI& spr, int ox, float t) {
  const float e = easeOutCubic(t);
  const Diag& d = diag();
  const uint32_t now = millis();
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    plStr(spr, PLF14, src[i].name, ox + 24, y0 + 11, col::TEXT);
...
void WeatherUi::streamScreenshot(WiFiClient& client, const WeatherModel& w, const PvModel& pv,
  ...
  // animacje rozjechałyby się między paskami.
  ...
  for (int top = HT - SHOT_H; top >= 0; top -= SHOT_H) {   // BMP idzie od dołu
    ...
    paintFrame(shot, w, pv, hist, fl, wifiOk, nowMs);
